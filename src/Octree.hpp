...
public:
    ...
    Octree(unsigned size) :
        ...
        leaf_count(0), branch_count(0), halfSize(size/2)
        {
            assert(isPow2(size));
            assert(size > 2);
        }
    ...
    void findNearestNeighbours(int x, int y,
                               int z, unsigned num,
                               std::vector<Leaf*> &leafs) const
        {
            assert(x <= width());
            assert(y <= width());
            assert(z <= width());
            ...
            NearestNeighboursSearchFull nns(x, y, z, num);
            nns.search(root(), 0, 0, 0, halfWidth());
            for(auto it:nns.leafs)
            {
                leafs.push_back(it.leaf);
            }
        }
...
private:
    ...
    // Nearest Neighbor Search in the Octree using bounding boxes.
    struct NearestNeighboursSearchFull
    {
        struct nnLeaf
        {
            // Nearest neighbour.
            Leaf* leaf;
            unsigned sq_distance;
            int x, y, z;
        };
        ...
        std::vector<nnLeaf> leafs;
        size_t maxLeafs;

        NearestNeighboursSearchFull(unsigned x, unsigned y, unsigned z, size_t maxLeafs) :
            pos_x(x), pos_y(y), pos_z(z),
            x_min(0), x_max(std::numeric_limits<int>::max()),
            y_min(0), y_max(std::numeric_limits<int>::max()),
            z_min(0), z_max(std::numeric_limits<int>::max()),
        ...
        void check_leaf(Leaf* leaf, int x, int y, int z)
            {
                ...
                nnLeaf l = {leaf, sq_distance, x, y, z};
                bool insert = false;
                for(int i=0; i<leafs.size(); ++i)
                {
                    if(sq_distance < leafs[i].sq_distance)
                    {
                        insert = true;
                        leafs.insert(leafs.begin()+i, l);
                ...
                if(!insert and leafs.size() < maxLeafs)
                {
                    leafs.push_back(l);
                    insert = true;
                }
                if(insert and leafs.size() == maxLeafs)
                {
                    const int r = std::sqrt(leafs[maxLeafs-1].sq_distance) + 1.0;
                    x_min = pos_x - r;
                    x_max = pos_x + r;
                    y_min = pos_y - r;
                    y_max = pos_y + r;
        ...
        bool check_branch(const unsigned x, const unsigned int y, const unsigned int z, const unsigned w) const
            {
                return !(x_max < (int)x || x_min > (int)(x+w) || y_max < (int)y || y_min > (int)(y+w) || z_max < (int)z || z_min > (int)(z+w));
            }
